...
The repository contains all of the [R](https://www.r-project.org/) code  used in the article. The is best seen by running the RMarkdown notebooks, within [RStudio](https://www.rstudio.com/). 
...
|`docs`|hmtl versions of the notebooks|
...
|`2indgps`|Compare 2 independent groups|Figure 9 (figure_2indgps.pdf)|
|`compcorr`|Comparison of correlation coefficients|Figure 10 (figure_compcorr.pdf)|
|`2depgps`|Illustrate hierarchical bootstrap sampling|Figure 11 (figure_hpb.pdf)|
|`ptb`|Percentile-t bootstrap technique|Figures 12-13 (figure_ptb.pdf, figure_ptb_explained.pdf)|
...
To install `rogme` and `facetscales`, you will need first to install `devtools`:
...
Finally, if you decide to run the simulations, you will `beepr` to get a little auditory reward:
`install.packages("beepr")` 
  
# Additional R functions 
Here we highlight a few R functions relevant to the tutorial. Most of them are listed in the RMarkdown notebooks, with example syntax. Each notebook will install the appropriate functions for you; otherwise, in the console you can type `source(file.choose())` and select the relevant txt files. 
...
[functions.txt](https://github.com/GRousselet/articles/blob/master/bootstrap/functions/functions.txt) and [theme_gar.txt](https://github.com/GRousselet/articles/blob/master/bootstrap/functions/theme_gar.txt) contain custom code to set some ggplot2 parameters and to compute a few things.
